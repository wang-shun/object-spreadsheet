...
let NotReadyError = Meteor.makeErrorType("NotReadyError", NotReadyError);
...
this.stringifyTypeForSheet = (type) => {
  var col, name;
  if (type === "_unit") {
    return "X";
...
this.markDisplayClassesForType = (type) => type === "_unit" ? ["centered"] : [];
...
class ClientView {
  ...
  public getDeleteCommandForCell(c) {
    ...
    } else if ((c.qCellId != null) && columnIsState(col = getColumn(c.qCellId.columnId))) {  // Should only exist in state value columns.
      return {

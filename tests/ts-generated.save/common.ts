...
this.rootColumnId = "_root";

// Multisets unsupported for now: twindex removed.

this.rootCellId = [];
this.cellIdParent = (cellId) => cellId.slice(0, -1);
this.cellIdChild = (cellId, value) => cellId.concat([value]);
this.cellIdLastStep = (cellId) => cellId[cellId.length - 1];
...
this.EvaluationError = Meteor.makeErrorType("EvaluationError", EvaluationError);

// Used also for typechecking.
class FormulaValidationError {
  constructor(public message) {}
}
this.FormulaValidationError = Meteor.makeErrorType("FormulaValidationError", FormulaValidationError);

class SyntaxError {
  constructor(public message, public details) {}
}
this.SyntaxError = Meteor.makeErrorType("SyntaxError", SyntaxError);

class SemanticError {
  constructor(public message) {}
}
this.SemanticError = Meteor.makeErrorType("SemanticError", SemanticError);
...
this.getColumn = (id) => Columns.findOne(id);

this.columnIsState = (col) => col._id !== rootColumnId && (col.formula == null);

this.objectNameWithFallback = (col) => fallback(col.objectName, (col.fieldName != null ? `[${col.fieldName}]` : null));

this.columnDepth = (columnId) => columnId === rootColumnId ? 0 : 1 + columnDepth(getColumn(columnId).parent);

this.nextAvailableColumnName = (prefix) => {
  let i = 1;
  while (Columns.find({
      $or: [
        {
...
this.columnLogicalChildrenByName = (id, wantName) => {
...
this.stringifyType = (type) => typeIsReference(type) ? stringifyColumnRef([type, false]) : type;
...
this.stringifyColumnRef = ([columnId, isValues]) => {
...
this.findCommonAncestorPaths = (columnId1, columnId2) => {
  var idx;
  let ancestors1 = [];
  let cid = columnId1;
  while (true) {
...
this.TYPE_EMPTY = "empty";

this.TYPE_ERROR = "error";

this.MAIN_PRIMITIVE_TYPES = ["text", "number", "bool", "date"];
this.DEFAULT_STATE_FIELD_TYPE = "text";
...
this.typeIsReference = (type) => type === rootColumnId || (!/^_/.test(type) && NON_REFERENCE_TYPES.indexOf(type) < 0);

this.commonSupertype = (t1, t2) => t1 !== TYPE_EMPTY ? t2 !== TYPE_EMPTY && t2 !== t1 ? TYPE_ERROR : t1 : t2;
...
this.allowedReferenceDisplayColumns = (col) => {
  let allowed = [];
...
this.defaultReferenceDisplayColumn = (col) => {
...
this.parseValue = (type, text) => {
  if (typeIsReference(type)) {
    ...
    } else {  // getColumn(type).referenceDisplay?
